...
class CylinderExpression(UserExpression):
    ...
    def eval(self, value, x):
        # Calculate theta relative to cylinder center
       
        theta = np.arctan2(x[1] - self.center_y, x[0] - self.center_x)
       
        if self.cylinder_type == "front":
            # Front cylinder (fixed): u = [0, 0]ᵀ
            value[0] = 0.0
            value[1] = 0.0
        elif self.cylinder_type == "top":
            # Top cylinder: u = b[sin θ, -cos θ]ᵀ
            value[0] = self.amplitude * np.sin(theta)
            value[1] = -self.amplitude * np.cos(theta)
        elif self.cylinder_type == "bottom":
            # Bottom cylinder: u = b[-sin θ, cos θ]ᵀ
            value[0] = -self.amplitude * np.sin(theta)
            value[1] = self.amplitude * np.cos(theta)
        # elif self.cylinder_type == "top":
        #     # Top cylinder: u = b[sin θ, -cos θ]ᵀ
...
def apply_boundary_conditions(W, mf, boundary_conditions, U_inlet=None):
    """
    ...
    Args:
        ...
        U_inlet: Inlet velocity profile function (optional)
    Returns:
    ...
    for bc in boundary_conditions:
        if bc['type'] == 'Dirichlet':
            ...
            if bc.get('bc_type') == 'cylinder':
                # Create cylinder expression
                expr = CylinderExpression(
                    center=bc['cylinder_center'],
                    cylinder_type=bc['cylinder_type'],
                    amplitude=bc['amplitude'],
                    degree=1
                )
                value = expr
            else:
